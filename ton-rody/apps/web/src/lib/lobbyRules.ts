...
export function getAllowedActions(ctx: LobbyContext): AllowedAction[] {
  ...
  if (ctx.state === 1) {
    ...
    if (ctx.isParticipant && now > ctx.joinDeadline && ctx.playersCount < 2) {
      actions.push('refund');
    }
  }
  ...
  if (ctx.state === 2) {
    ...
    if (now > ctx.revealDeadline || ctx.playersCount === ctx.maxPlayers) {
      actions.push('finalize');
    }
    // Refund after cancellation (handled separately)
  }
