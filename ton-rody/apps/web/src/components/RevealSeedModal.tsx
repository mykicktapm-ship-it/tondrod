...
import { loadSeed, deleteSeed, exportSeeds } from '../lib/seedVault';
import { buildRevealPayload } from '../lib/ton';

interface RevealSeedModalProps {
  lobbyAddress: string;
  lobbyId: number;
  walletAddress: string;
  onReveal: (payload: string) => void;
  onCancel: () => void;
}
...
export default function RevealSeedModal({
  ...
  const handleReveal = async () => {
    if (!secret) return;
    // convert secret hex into BigInt for payload
    let bn: bigint;
    try {
      bn = BigInt('0x' + secret);
    } catch {
      // fallback to zero to prevent accidental reveal
      bn = 0n;
    }
    const payload = buildRevealPayload(bn);
    onReveal(payload);
  };
        ...
        {loading ? (
          ...
          <>
            <p className="text-sm mb-4">
              The secret for this lobby and wallet could not be found on
              this device. Without the secret you cannot reveal, so you
              may lose your stake. If you joined on another device you
              should import the backup file before revealing.
            </p>
            {exportData ? (
              <textarea
